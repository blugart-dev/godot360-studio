...
var exposure_control: OptionButton
...
func _refresh_fields() -> void:
	...
	exposure_control.select(preload("capture_exposure.gd").MODES.find(profile.capture_exposure_mode))
	_refresh_audio_fields()
	_refresh_quality_hint()
	_refresh_scene_cameras(false)
...
func _refresh_quality_hint() -> void:
	...
	if border_hint != null and int(recipe_fields.face_size.text) > 0:
		...
		border_hint.text = "Extra scene context can reduce glow cuts at face edges. %d px face targets, %.0f%% more face pixels. Render time and GPU memory may increase; run a new short test. Auto exposure is controlled separately below." % [geometry.texture_size, (geometry.pixel_ratio - 1.0) * 100.0]
...
func _save_settings() -> void:
	...
	for key in ["crf", "random_seed", "warmup_frames", "frame_writer", "rendering_method", "rendering_driver", "capture_border_percent", "capture_exposure_mode"]:

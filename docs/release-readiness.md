...
| Complex animated scenes | [Keyed skin and bone-camera cuts](skeletal-capture.md), [an imported GLB character](imported-characters.md) and [simple particle startup/motion/pausing](particle-capture.md) have rendered references. Fixed opening deltas and Compatibility automatic bounds are implemented; the simple particle contract uses at least two warmup frames. Remaining: complex particle effects, modifiers/IK, nested skeleton attachments, other import/retarget pipelines, longer temporal histories, FSR, VoxelGI/LightmapGI and stateful compositors. | Deterministic fixtures with expected motion and visual comparisons; supported cases pass and exclusions are explicit. |
...
1. Broaden scene fixtures: modifiers/IK, nested attachments, complex particles and

"""Render LUMEN in an isolated project through the Godot360 delivery pipeline.

The portable editor recipe is export_profiles/lumen-4k.tres. This helper uses
...
def render(args):
    ...
    assert 0 < args.seconds <= 24 and args.face_size >= 128
    output.mkdir(parents=True, exist_ok=False)
    project = output / 'project'
    files = ['scripts/films/lumen.gd', 'scripts/films/threshold_geometry.gd',
             'scenes/films/Lumen.tscn', 'assets/films/lumen/sky.gdshader',
             'assets/films/lumen/core.gdshader', 'assets/films/lumen/floor.gdshader',
             'assets/audio/lumen-score.wav']
    protected = ['project.godot', '.godot360/settings.cfg', 'scenes/Main.tscn',
                 'scenes/films/Threshold.tscn', 'export_profiles/threshold-8k.tres']
...
[application]
config/name="LUMEN / Godot360"
run/main_scene="res://scenes/films/Lumen.tscn"
[display]
...
''')
    ...
    job = {'scene_path': 'res://scenes/films/Lumen.tscn', 'camera_path': 'Camera3D',
           'width': args.width, 'height': args.width // 2, 'face_size': args.face_size,
           'fps': 30, 'frames': round(args.seconds * 30), 'warmup_frames': 2,
           'capture_border_percent': 12.5, 'frame_writer': 'fast_png', 'crf': 16,
           'rendering_method': 'forward_plus', 'rendering_driver': args.driver,
           'random_seed': 9360, 'audio_mode': 'soundtrack',
           'soundtrack_path': 'res://assets/audio/lumen-score.wav',
    ...
    with (output / 'pipeline.log').open('w') as log:
        ...
        try:
            ...
            if process.poll() is None:
                (destination / 'cancel.request').write_text('LUMEN monitor stopped the render')
                try:

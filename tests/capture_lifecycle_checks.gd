...
func _run() -> void:
	...
	var renderer_script := folder.path_join("renderer-error.gd")
	FileAccess.open(renderer_script, FileAccess.WRITE).store_string("extends Node3D\nfunc _ready() -> void:\n\tprint(\"ERROR: Image needs the TEXTURE_USAGE_STORAGE_BIT usage flag.\\n   at: uniform_set_create (servers/rendering/rendering_device.cpp:4555)\")\n")
	var renderer_scene := folder.path_join("renderer-error.tscn")
	FileAccess.open(renderer_scene, FileAccess.WRITE).store_string(FileAccess.get_file_as_string(hook_scene).replace(ProjectSettings.localize_path(hook), ProjectSettings.localize_path(renderer_script)))
	var renderer_failure := await _job("renderer-error", {"scene_path": ProjectSettings.localize_path(renderer_scene), "frames": 3})
	check(str(renderer_failure.capture.get("error", "")).contains("Renderer failed during capture"), "A complete frame sequence with an engine rendering error cannot pass capture")
	check(int(renderer_failure.capture.get("rendered", -1)) == 5, "Rendering-error rejection also applies after every requested frame was written")
